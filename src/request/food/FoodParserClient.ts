...
export default class FoodParserClient extends IFoodClient {
    ...
    static getRequestUrl({ diningHall, menuDate, meal } : IMenuSelection): string {
        return encodeURI(`${StringUtil.joinUrl(paths.eatAtState.base, paths.eatAtState.menu, diningHall.fullName)}/all/${menuDate.getFormatted()}?field_mealtime_target_id=${MealIdentifier[meal]}`);
    }
